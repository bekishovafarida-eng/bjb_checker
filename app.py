...
if not OPENAI_API_KEY:
    raise ValueError("OPENAI_API_KEY табылмады")
...
def parse_json_response(text):
    ...
    try:
        ...
        raise ValueError("JSON оқу кезінде қате шықты: " + text)

# ===== CHATGPT JSON FUNCTION =====
def ask_chatgpt_json(prompt):
    response = openai_client.chat.completions.create(
        model="gpt-4o-mini",
        messages=[
            {
                "role": "system",
                "content": "Сен қазақ тілінде жауап беретін мұғалім көмекшісісің. Жауапты тек таза JSON форматында қайтар. Markdown қолданба."
            },
            {
                "role": "user",
                "content": prompt
...
# ===== БЖБ ТЕКСЕРУ =====
def check_with_chatgpt(student_text, answer_key, max_score):
    prompt = f"""
...
"""
    return ask_chatgpt_json(prompt)
...
# ===== ЭССЕ ТЕКСЕРУ =====
def check_essay_with_chatgpt(recognized_text, topic="", grade_level="", word_count=""):
    prompt = f"""
...
{{
  ...
  "corrected_version": "Түзетілген қысқа нұсқа"
}}
"""
    return ask_chatgpt_json(prompt)
...
def save_to_word(student_text, result, max_score):
    ...
    doc.add_paragraph(f"Ұпай: {result.get('score', 0)} / {max_score}")
    doc.add_paragraph(f"Қорытынды: {result.get('decision', '')}")
    doc.add_paragraph(f"Кері байланыс: {result.get('feedback', '')}")
...
def bjb():
    ...
    if request.method == "POST":
        try:
            ...
            result = check_with_chatgpt(student_text, answer_key, max_score)
...
def essay_check():
    ...
    if request.method == "POST":
        try:
            ...
            essay_check_result = check_essay_with_chatgpt(
                recognized_text,
                topic=topic,
    ...
    return render_template(
        "essay_check.html",
        essay_check_result=essay_check_result,
        error=error
    )
